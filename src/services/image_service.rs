...
fn calculate_user_hash(user: &DiscordUser) -> String {
    ...
    for stat in &user.stats {
        ...
        if let Some(channels) = &stat.voice_channels {
            for ch in channels { hasher.update(ch.name.as_bytes()); }
        }
        if let Some(channels) = &stat.text_channels {
            for ch in channels { hasher.update(ch.name.as_bytes()); }
        }
        if let Some(vocal_with) = &stat.vocal_with {
            for comp in vocal_with { hasher.update(comp.username.as_bytes()); }
        }
    }
...
pub async fn generate_discord_profile(user: DiscordUser) -> ImageResponse {
    ...
    for x in 0..SIDEBAR_WIDTH {
        for y in 0..CARD_HEIGHT { img.put_pixel(x, y, sidebar_bg); }
    }
    for y in 0..CARD_HEIGHT { img.put_pixel(SIDEBAR_WIDTH, y, Rgba(COLOR_SEPARATOR)); }
    for x in 0..SIDEBAR_WIDTH {
        for y in 0..6 { img.put_pixel(x, y, accent_blurple); }
    }
    ...
    draw_text_centered_rgba(&mut img, &font, &user.discord_tag, 150, 325, Scale::uniform(22.0), gray_label);
    ...
    for stat in &user.stats {
        if let Some(channels) = &stat.text_channels {
            for ch in channels { *text_counts.entry(ch.name.clone()).or_insert(0) += 1; }
        }
        if let Some(channels) = &stat.voice_channels {
            for ch in channels { *voice_counts.entry(ch.name.clone()).or_insert(0) += 1; }
        }
        if let Some(vocal_with) = &stat.vocal_with {
            for comp in vocal_with { *companion_counts.entry(comp.username.clone()).or_insert(0) += 1; }
        }
    }
    ...
    let filtered_roles: Vec<_> = user.roles.iter().filter(|r| r.name.to_lowercase().contains("loutre") || r.name.to_lowercase().contains("rôle")).take(8).collect();
    ...
    draw_stat_card(&mut img, Rect { x: grid_x, y: grid_y, w: card_w, h: card_h }, LABEL_MESSAGES, &total_messages.to_string(), &font);
    draw_stat_card(&mut img, Rect { x: grid_x + 370, y: grid_y, w: card_w, h: card_h }, LABEL_VOCAL_TIME, &formatters::format_vocal_time(total_vocal_decimal), &font);
    draw_stat_card(&mut img, Rect { x: grid_x, y: grid_y + 150, w: card_w, h: card_h }, LABEL_COMPANION, &formatters::truncate_text(&top_companion, 20), &font);
    draw_stat_card(&mut img, Rect { x: grid_x + 370, y: grid_y + 150, w: card_w, h: card_h }, LABEL_TEXT_CHANNEL, &formatters::truncate_text(&top_text, 15), &font);
    draw_stat_card(&mut img, Rect { x: grid_x, y: grid_y + 300, w: 710, h: card_h }, LABEL_VOICE_CHANNEL, &formatters::truncate_text(&top_voice, 40), &font);
...
fn draw_stat_card(img: &mut RgbaImage, rect: Rect, label: &str, val: &str, font: &Font) {
    ...
    for bx in rect.x..(rect.x + rect.w) {
        for by in rect.y..(rect.y + rect.h) {
            if bx < img.width() && by < img.height() { img.put_pixel(bx, by, card_bg); }
        }
    }
    for bx in rect.x..(rect.x + rect.w) { img.put_pixel(bx, rect.y + rect.h - 1, Rgba([88, 101, 242, 100])); }
    draw_text_rgba(img, font, label, (rect.x + 25) as i32, (rect.y + 25) as i32, Scale::uniform(15.0), gray_label);
    draw_text_rgba(img, font, val, (rect.x + 25) as i32, (rect.y + 55) as i32, Scale::uniform(36.0), white);
}
        ...
        for dy in 0..height {
            ...
            if dist <= radius {
                ...
                if dist > radius - border_width {
                    ...
                    target_color = Rgba([(color.0[0] as f32 * alpha + inner_bg.0[0] as f32 * inv_alpha) as u8, (color.0[1] as f32 * alpha + inner_bg.0[1] as f32 * inv_alpha) as u8, (color.0[2] as f32 * alpha + inner_bg.0[2] as f32 * inv_alpha) as u8, 255]);
                }
                if dist > radius - 1.0 {
                ...
                } else { img.put_pixel(px as u32, py as u32, target_color); }
            }
        }
    }
