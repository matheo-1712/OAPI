...
use crate::models::{MinecraftPlayer, MinecraftStats, PlayerBadge, Server, ImageResponse};
use crate::services;
use crate::utils::constants::{
    MINECRAFT_PLAYERS_COLLECTION, MINECRAFT_STATS_COLLECTION, MINECRAFT_SERVERS_COLLECTION,
    DISCORD_USER_BADGES_COLLECTION,
};
...
async fn fetch_minecraft_data(uuid: &str) -> Result<MinecraftPlayer, String> {
    debug!("Action: Fetching Minecraft data from PocketBase for UUID: {}", uuid);
    ...
    let players: Vec<MinecraftPlayer> = pb.list_all_records(MINECRAFT_PLAYERS_COLLECTION, &filter_user).await?;
    let mut player = players.into_iter().next().ok_or_else(|| format!("Player not found with UUID: {}", uuid))?;
    ...
    let stats: Vec<MinecraftStats> = pb.list_all_records(MINECRAFT_STATS_COLLECTION, &filter_stats).await?;
    ...
    for mut val in raw_badges {
        if let Some(obj) = val.as_object_mut() {
            if let Some(badge_info) = obj.get("expand").and_then(|e| e.get("badge")).cloned() {
                ...
                warn!("Badge record missing 'expand.badge' for Minecraft: {:?}", obj.get("id"));
            }
        }
        match serde_json::from_value::<PlayerBadge>(val.clone()) {
            Ok(badge) => processed_badges.push(badge),
            Err(e) => {
                error!("Failed to deserialize Minecraft badge: {}. Raw value: {:?}", e, val);
            }
        }
    }

    // 4. Fetch all servers to resolve names and colors
    let servers: Vec<Server> = pb.list_all_records(MINECRAFT_SERVERS_COLLECTION, "").await?;
    ...
    player.badges = if processed_badges.is_empty() { None } else { Some(processed_badges) };
